...
import com.chiraitori.ganyu.databinding.ActivityMainBinding
import com.chiraitori.ganyu.lib.Intents
import com.chiraitori.ganyu.lib.NavigationData
import com.chiraitori.ganyu.service.BleService
import com.chiraitori.ganyu.ui.ActivityViewModel
import com.chiraitori.ganyu.ui.BleDeviceSelectionActivity
import com.chiraitori.ganyu.utils.PermissionCheck
...
class MainActivity : AppCompatActivity() {
    ...
    private val navigationReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent) {
            val data = intent.getParcelableExtra<NavigationData?>("navigation_data")
    ...
    private fun checkPermissions() {
        if (!PermissionCheck.checkNotificationsAccessPermission(this)) {
            Toast.makeText(
                this,
                "Please grant notification access to CatDrive",
                Toast.LENGTH_LONG
            ).show()
            val intent = Intent(Intents.OPEN_NOTIFICATION_LISTENER_SETTINGS)
            startActivity(intent)
        } else if (!PermissionCheck.allPermissionsGranted(this)) {
            Toast.makeText(
                this,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val appBarConfiguration = AppBarConfiguration(
            setOf(
                R.id.navigation_product, R.id.navigation_home, R.id.navigation_settings
            )
        )
        // No ActionBar needed for Moondrop-style UI
    ...
    override fun onStart() {
        ...
        // Auto-enable services on app start
        ServiceManager.startBroadcastService(this)

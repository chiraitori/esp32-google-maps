...
import com.chiraitori.ganyu.lib.BleCharacteristics
import com.chiraitori.ganyu.lib.BleWriteQueue
import com.chiraitori.ganyu.lib.BleWriteQueue.QueueItem
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                val deviceName = mDevice?.name ?: "Unknown Device"
                val deviceAddress = mDevice?.address ?: "Unknown"
                Timber.i("Connected successfully - Device: $deviceName ($deviceAddress)")
                
                updateNotificationText("Connected to $deviceName")
                stopReconnectTimer()
                startPingTimer()
                sendPreferencesToDevice()
...
//            Timber.e("write: not connected")
    ...
    fun connect(device: BluetoothDevice) {
        Timber.i("Connecting to device: name=${device.name}, address=${device.address}")
        mDevice = device
        mBluetoothGatt =
    ...
    private fun buildForegroundNotification(): Notification {
        ...
        mNotificationBuilder = builder.setContentTitle("Ganyu Drive")

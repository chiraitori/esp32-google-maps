...
internal class GMapsNotification(cx: Context, sbn: StatusBarNotification) : NavigationNotification(cx, sbn) {
    init {
        try {
            // Debug: Log notification extras for Android 13+
            val extras = sbn.notification.extras
            Timber.d("=== Google Maps Notification Debug ===")
            Timber.d("Notification ID: ${sbn.id}")
            Timber.d("Title: ${extras.getCharSequence(android.app.Notification.EXTRA_TITLE)}")
            Timber.d("Text: ${extras.getCharSequence(android.app.Notification.EXTRA_TEXT)}")
            Timber.d("SubText: ${extras.getCharSequence(android.app.Notification.EXTRA_SUB_TEXT)}")
            Timber.d("BigText: ${extras.getCharSequence(android.app.Notification.EXTRA_BIG_TEXT)}")
            Timber.d("Info Text: ${extras.getCharSequence(android.app.Notification.EXTRA_INFO_TEXT)}")
            ...
            Timber.e("Failed to parse Google Maps notification: $e")
            e.printStackTrace()
        }
    }
